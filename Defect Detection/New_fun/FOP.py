...
class App():
    def __init__(self):
        ...
        self.plt_setting()
        Button(self.root, text='draw', command=self.multi_draw).grid(row=1, column=2, columnspan=3)
    ...
    def plt_setting(self):
        ...
        self.canvas = FigureCanvasTkAgg(self.f, master=self.root)# #create canvas to be the container of Figure
        self.canvas.get_tk_widget().grid(row=0, columnspan=3)#to place the canvas in self.root
    def multi_draw(self):
        for i in range(3):
            self.draw_RGB()
            time.sleep(1)
