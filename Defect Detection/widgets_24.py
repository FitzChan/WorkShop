...
################### 第 2 页 用到方法################################################
    ...
    # 处理照片
    def process_pic(self,value_Brightness,value_Threshold_constant,value_Convolution_kernel,value_Lower_defect_area,info_st,fileEntry,dstEntry,canvas,queue):
        #1 get params
        #queue = Queue()
        ...
        return_list = [dst_folder_add, self.defect_src_list]
...
################### 第 3 页 用到方法################################################
    ...
    # # 循环播放处理后的图片
    # def dst_set(self,canvas_dst,pic_seq):
    #     defect_img_list, defect_list_len = self.get_defect_img_list(self.dst_folder_address)
    #     dst = cv.imread(defect_img_list[pic_seq])
    #     dst_rgb = cv.cvtColor(dst, cv.COLOR_BGR2RGB)
    #     global dst_PIL
    #     dst_PIL = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(dst_rgb))
    #     canvas_dst.create_image(0, 0, image=dst_PIL, anchor=tk.NW)
    #
    # def src_set(self,canvas_src,pic_seq):
    #     if self.defect_src_list:
    #         src = cv.imread(self.defect_src_list[pic_seq])
    #         src_rgb = cv.cvtColor(src, cv.COLOR_BGR2RGB)
    #         global src_PIL
    #         src_PIL = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(src_rgb))  # image 需为全局变量
    #         canvas_src.create_image(0, 0, image=src_PIL, anchor=tk.NW)
    # def paging_next(self,canvas_src,canvas_dst,pic_seq):
    #     if pic_seq < len(self.defect_src_list) - 1:
    #         pic_seq += 1
    #         self.src_set(canvas_src,pic_seq)
    #         self.dst_set(canvas_dst,pic_seq)
    #         return pic_seq
    # # 缺陷照片翻页 上一页
    # def paging_last(self,canvas_src,canvas_dst,pic_seq):
    #     if pic_seq > 0:
    #         pic_seq -= 1
    #         self.src_set(canvas_src,pic_seq)
    #         self.dst_set(canvas_dst,pic_seq)
