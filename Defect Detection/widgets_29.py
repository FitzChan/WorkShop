...
class MyVideoCapture:  ## 读取视频 及 其高宽
    def __init__(self, canvas_width,canvas_height,video_source= 0,):
        ...
        self.vid.set(3, 2592) #2592 大恒相机分辨率
        self.vid.set(4, 2048) #2048
    ...
    def get_frame(self):
        if self.vid.isOpened():
            ...
            frame = cv.resize(frame, (self.width, self.height),interpolation=cv.INTER_AREA)# 采用最快的临近点差值
            frame_height = frame.shape[0]
            #print(frame_height)  #432
            frame_width = frame.shape[1]
            #print(frame_width) #576
            #roi_diagonal= 720
            roi_height = int(frame_height/1.4142)#将圆形区域显示变更为正方形
            roi_width = int(frame_width/1.4142)
            roi = frame[int((frame_height-roi_height)/2):int((frame_height+roi_height)/2),int((frame_width-roi_width)/2):int((frame_width+roi_width)/2)]
            if ret:
                 #frame = cv.flip(frame, 1)
                 #roi = cv.flip(roi, 1)
                 roi = cv.resize(roi, (self.width, self.height),interpolation=cv.INTER_LINEAR)
                 #return (ret, cv.cvtColor(frame, cv.COLOR_BGR2RGB))
                 return (ret, cv.cvtColor(roi, cv.COLOR_BGR2RGB))
            else:
                 return (ret, None)
        else:
             ret = 2
