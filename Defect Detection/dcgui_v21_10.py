...
################### 第 1 页 用到方法################################################
    ...
    def get_pic(self):
        ...
        self.create_dst_folder(folder_time)
        while shot_flag:
...
################### 第 2 页 用到方法################################################
    ...
    def create_dst_folder(self,folder_time,flag =0): #每次拍摄都创建一个文件夹用于保存dst图片
        ...
        self.dst_folder_path = folder_path_head +'/'+ folder_time # 文件夹用时间来命名
        if flag ==0:
            makedirs(self.dst_folder_path )
    #保存处理过的图片
    def savedstPic(self,pic):
        print('fName replace before', self.fName)
        fName = self.fName.replace('D:/Defect Detection/','')
        fName = fName.replace('\\', '/')
        print('fName',fName)
        dst_address = self.dstEntry.get()
        full_dst_name = dst_address + '/dst_'+fName
...
################### 第 3 页 用到方法################################################
    ...
    def get_defect_img_list(self):
        read_pic_add = self.dstEntry.get()
        glob_add = read_pic_add + '/*.jpg'
    ...
    def put_defect_img(self):
        defect_img_list,defect_list_len = self.get_defect_img_list()
        if defect_img_list != 0:
            ...
            self.canvas3.after(1000,self.put_defect_img)
