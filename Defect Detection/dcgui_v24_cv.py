...
import widgets_24 # 包含 MyVideoCapture
...
class App:
    def __init__(self, win, window_title,video_source = 0):
        ...
        # open video source
        self.vid = widgets_24.MyVideoCapture()
        self.widfun = widgets_24.WidFun()
        ...
        #self.widgetsdefaultFileEntries()
...
#######################创建 第 1 页的 按钮 和 标签 #######################################
    def create_widgets(self):
        ...
        self.button_font = font.Font(family='Symbol',size = 12)
        ...
        but_open_com.place(relx = 0.45,rely = 0.03)
        ...
        but_start_motor_program.place(relx=0.55, rely=0.03)
        ...
        but_get_pic.place(relx = 0.45 , rely = 0.11)
        ...
        but_click_move.place(relx=0.45, rely=0.19)
        ...
        combobox_click_move_pos.place(relx=0.55, rely=0.19)
        ...
        self.canvas.place(relx = 0.45 , rely = 0.285)
...
############################ 设置第 3 页 用到的 widgets###########################
    def create_widgets_3(self):
        ...
        self.defect_info_st = scrolledtext.ScrolledText(self.tab_defect,width = 40 , height = 15,wrap=tk.WORD)
        self.defect_info_st.place(relx = 0.02 , rely = 0.10 )
        write_but = tk.Button(self.tab_defect, text="保存缺陷信息", width = 12,relief = GROOVE,font = self.button_font,fg = 'black',bg = '#DCDCDC',command = self.write_defect_log)
...
################### 第 1 页 用到方法################################################
    ...
    def plt_setting(self):
        self.f = Figure(figsize=(5, 4), dpi=110)
        self.canvas_plt = FigureCanvasTkAgg(self.f, master=self.tab_video)
        self.canvas_plt.get_tk_widget().place(relx = 0.01,rely = 0.345)
    # 用于1次拍照
...
################### 第 2 页 用到方法################################################
    ...
    def creat_thread_process_pic(self):
        ...
        args = (self.value_Brightness,self.value_Threshold_constant,self.value_Convolution_kernel,self.value_Lower_defect_area,self.defect_info_st,self.fileEntry,self.dstEntry,self.canvas3,q)
        runThread = Thread(target=self.widfun.process_pic(self.value_Brightness,self.value_Threshold_constant,self.value_Convolution_kernel,self.value_Lower_defect_area,self.defect_info_st,self.fileEntry,self.dstEntry,self.canvas3,q))
